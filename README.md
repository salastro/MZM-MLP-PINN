# MLP Hyperparameter Tuning — MZM PINN (PyTorch)

> **Branch:** `hyper/mlp` — diverges from `master`
> JAX/Flax implementation of this search: `hyper/mlp-jax`

Bayesian hyperparameter search for the **Multi-Layer Perceptron (MLP)** surrogate of Mach-Zehnder Modulators, using **PyTorch** and **Optuna**.

## Overview

MZMs are essential components in optical communication systems. This branch tunes the hyperparameters of a Physics-Informed MLP surrogate that maps device geometry to key performance metrics, with physics-based soft constraints enforced via PyTorch autograd.
...
A 5-layer feedforward network with BatchNorm, dropout, and residual connections. Hyperparameters searched via Optuna (TPE sampler, median pruner):
...
2. Open `MZM_Hyperparameter_Tuning_MLP.ipynb` and run all cells.
